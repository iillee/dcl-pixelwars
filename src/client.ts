...
// SeedHolder component + entity moved to src/shared/components.ts — must be
// statically imported from index.ts before the engine seals. See that file
// for the full rule. Imported above alongside `seedHolder` for use below.
...
export async function setupClient() {
  // ─── Phase 4 Step 1: server hello-world roundtrip ─────────────────
  // Prove the auth server is up and WS is flowing both directions.
  // Sends one ping ~1s after client boot, logs the pong we get back.
  room.onMessage('pong', ({ t, serverT }) => {
    const rtt = Date.now() - t
    console.log(`[Client] pong received — rtt ${rtt}ms, serverT ${serverT}`)
  })
  let pingSent = false
  let pingTimer = 0
  engine.addSystem((dt: number) => {
    if (pingSent) return
    pingTimer += dt
    if (pingTimer < 1.0) return
    pingSent = true
    const t = Date.now()
    console.log(`[Client] → ping t=${t}`)
    room.send('ping', { t })
  })
